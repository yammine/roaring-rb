...
namespace :doc do
  ...
  task :rustdoc do
    sh <<~CMD
      rustup run nightly cargo rustdoc --target-dir tmp/doc/target -p roaring_rb --manifest-path ext/roaring/Cargo.toml -- -Zunstable-options --output-format json --document-private-items
    CMD

    cp "tmp/doc/target/doc/roaring_rb.json", "tmp/doc/roaring_rb.json"
  end
end
